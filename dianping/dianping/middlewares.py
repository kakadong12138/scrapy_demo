...
class AdslMiddleware(object):
    ...
    def process_response(self,request, response, spider):
        ...
        if response.status != 200:
            self.times_403+=1
            if  self.times_403 >settings.CONCURRENT_REQUESTS + 1 and self.sailing == True:
                print 'sit 1'
                self.sailing = False
                os.system('adsl-stop')
    ...
    def process_exception(self,request, exception, spider):
        ...
        print "self.exception_times changed is %s"%(self.exception_times)
        if self.exception_times > settings.CONCURRENT_REQUESTS + 1 and self.sailing == True:
            self.sailing = False
            os.system('adsl-stop')
            os.system('adsl-start')
            self.sailing = True
